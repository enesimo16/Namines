...
import { X, CloudLightning, RefreshCw, Trash2, FolderGit2 } from 'lucide-react';
      ...
      <div className="relative w-full max-w-md bg-[#09111F]/95 backdrop-blur-2xl border border-emerald-500/20 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.85)] overflow-hidden flex flex-col p-6 pb-8 animate-in zoom-in-95 duration-200">
        ...
        <div className="my-5 flex flex-col gap-2">
          <span className="text-sm font-bold text-zinc-100 leading-snug">
            🗂️ You have {projects.length} project{projects.length > 1 ? 's' : ''} from your guest session
          </span>
        ...
        {/* Local Projects List */}
        <div className="max-h-40 overflow-y-auto mb-6 bg-zinc-950/40 border border-zinc-800/80 rounded-2xl p-3 flex flex-col gap-2 custom-scrollbar">
          {projects.map((proj) => {
            ...
            return (
              <div 
                key={proj.id} 
                className="flex items-center justify-between p-2.5 rounded-xl bg-zinc-900/60 border border-zinc-800/50 hover:border-zinc-700/50 transition-all duration-200"
              >
        ...
        <div className="flex gap-3">
          <button
            onClick={onDiscard}
            className="flex-1 py-3 px-4 rounded-xl border border-zinc-700/60 bg-zinc-800/40 hover:bg-zinc-800 text-zinc-300 hover:text-white font-bold text-xs tracking-wider uppercase transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer"
          >
          ...
          <button
            onClick={onSync}
            className="flex-1 py-3 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-xs tracking-wider uppercase transition-all duration-300 shadow-[0_4px_15px_rgba(16,185,129,0.3)] hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2 cursor-pointer"
          >
