...
export default function VisionUploadModal({ isOpen, onClose }: VisionUploadModalProps) {
  ...
  const processFile = (file: File) => {
    ...
    if (!validTypes.includes(file.type)) {
      setError('Geçersiz dosya formatı. Lütfen JPEG, PNG veya WebP yükleyin.');
      return;
    }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (!schema || !schema.tables || schema.tables.length === 0) {
        throw new Error('Görselden geçerli bir veritabanı şeması çözümlenemedi.');
      }
      ...
      setError(err?.response?.data?.error || err.message || 'Görsel çözümlenirken AI sunucusunda bir hata oluştu.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative w-full max-w-lg rounded-3xl bg-[#0F172A]/95 backdrop-blur-md border border-indigo-500/20 shadow-[0_0_50px_rgba(99,102,241,0.25)] p-6 md:p-8 flex flex-col max-h-[85vh] overflow-hidden">
        ...
        <div className="flex justify-between items-start mb-6 shrink-0">
          <div>
            <h2 className="text-xl font-bold bg-gradient-to-r from-indigo-200 via-indigo-400 to-indigo-100 bg-clip-text text-transparent flex items-center gap-2">
              ...
              <span>{showVerification ? 'Şema Doğrulama' : 'Beyaz Tahtadan İçe Aktar'}</span>
            </h2>
            <p className="text-zinc-400 text-xs mt-1">
              {showVerification 
                ? 'AI tarafından tespit edilen tablo ve yabancı anahtarları gözden geçirin.' 
                : 'Fiziksel beyaz tahtaya çizdiğiniz veritabanı şemasının fotoğrafını yükleyin, AI ile anında dijital tablolara dönüştürelim.'}
            </p>
          </div>
          <button 
            onClick={handleClose}
        ...
        {error && (
          <div className="bg-rose-950/20 border border-rose-500/20 rounded-2xl p-4 flex gap-3 items-start mb-5 shrink-0 animate-in fade-in slide-in-from-top-2">
            ...
            <div>
              <span className="text-rose-400 text-xs font-bold block mb-0.5">Analiz Başarısız Oldu</span>
        ...
        {success && (
          <div className="bg-emerald-950/20 border border-emerald-500/20 rounded-2xl p-4 flex gap-3 items-start mb-5 shrink-0 animate-in fade-in">
            ...
            <div>
              <span className="text-emerald-400 text-xs font-bold block mb-0.5">Şema Başarıyla Aktarıldı!</span>
              <p className="text-zinc-300 text-[11px] leading-relaxed">Tablolar ve ilişkiler algılanarak alt satırlara yerleştirildi. Canvas güncelleniyor...</p>
            </div>
          </div>
        )}
          ...
          {showVerification && parsedSchema ? (
            ...
            <div className="space-y-4 py-2 animate-in fade-in slide-in-from-bottom-2">
              <div className="bg-indigo-500/5 border border-indigo-500/10 rounded-2xl p-4 flex gap-3 items-start">
                ...
                <div>
                  <span className="text-indigo-300 text-xs font-bold block mb-0.5">Çözümleme Başarılı!</span>
                  <p className="text-zinc-400 text-[10px] leading-relaxed">
                    Aşağıdaki algılanan tablo yapısını ve ilişkileri gözden geçirin. Hatalı eşleştirilmiş ilişkilerin onay kutusunu kaldırarak aktarıma kapatabilirsiniz.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider block">Algılanan Tablolar ({parsedSchema.tables.length})</span>
                <div className="grid grid-cols-2 gap-2 max-h-[110px] overflow-y-auto pr-1 custom-scrollbar">
                  {parsedSchema.tables.map((t: any) => (
                    <div key={t.id} className="p-2.5 bg-zinc-950/40 border border-zinc-900 rounded-xl flex justify-between items-center text-xs">
                      ...
                      <span className="text-zinc-500 text-[10px]">{t.columns?.length || 0} Kolon</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-2">
                <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider block">Algılanan İlişkiler (Foreign Keys - {parsedSchema.relations?.length || 0})</span>
                <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1 custom-scrollbar">
                  {(!parsedSchema.relations || parsedSchema.relations.length === 0) ? (
                    <p className="text-zinc-500 text-xs italic p-3 bg-zinc-950/20 border border-zinc-900 rounded-xl text-center">Herhangi bir ilişki algılanamadı.</p>
                  ) : (
            ...
            <div 
              ...
              className={`border-2 border-dashed rounded-3xl p-8 flex flex-col items-center justify-center gap-3 cursor-pointer transition-all duration-300 min-h-[200px] ${isDragOver ? 'border-indigo-400 bg-indigo-500/5' : 'border-zinc-800 hover:border-zinc-700 bg-zinc-950/20'}`}
            >
              <input 
                type="file" 
                ref={fileInputRef}
              ...
              <div className="text-center">
                <span className="text-zinc-200 text-sm font-bold block">Resim Dosyanızı Sürükleyin</span>
                <span className="text-zinc-400 text-xs mt-1 block">veya tıklayarak cihazınızdan seçin (JPEG, PNG, WebP - Max 10MB)</span>
              </div>
            </div>
          ) : (
            /* Preview Area */
            <div className="space-y-4">
              <div className="relative rounded-2xl overflow-hidden border border-zinc-800 bg-zinc-950 max-h-[250px] flex items-center justify-center shadow-inner">
                ...
                {loading && (
                  ...
                  <div className="absolute inset-0 bg-black/75 backdrop-blur-sm flex flex-col items-center justify-center gap-3 animate-in fade-in duration-300">
                    <RefreshCw className="w-8 h-8 text-indigo-400 animate-spin" />
                    <div className="text-center px-4">
                      <span className="text-zinc-100 text-xs font-bold block animate-pulse">Şema Çözümleniyor...</span>
                      <span className="text-zinc-400 text-[10px] mt-1 block">AI görseli analiz ediyor, el yazıları ve çizgiler okunuyor.</span>
                    </div>
                  </div>
                )}
              </div>
        ...
        {/* Footer */}
        <div className="border-t border-zinc-900 pt-6 mt-6 flex justify-end gap-3 shrink-0">
          <button 
            onClick={handleClose}
            disabled={loading}
          ...
          {showVerification && parsedSchema && !success && (
            ...
            >
              ...
              <span>Onayla ve Canvas'a Aktar</span>
            </button>
          )}
