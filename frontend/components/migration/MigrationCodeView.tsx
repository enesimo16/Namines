  ...
  return (
    <div className="space-y-4">
      ...
      {(hasBreakingChanges || (migration?.warnings && migration.warnings.length > 0)) && (
        <div className="bg-danger-subtle border border-danger/25 rounded-[var(--radius-card)] p-3.5 space-y-2">
      ...
      {/* Summary Box */}
      {migration?.summary && (
        <div className="bg-surface-700 rounded-[var(--radius-card)] p-3 flex gap-2.5 items-start">
      ...
      <div className="flex items-center justify-between border-b border-content-primary/8 pb-2">
        <div className="flex gap-1 bg-surface-700 p-1 rounded-[var(--radius-control)]">
          <button
            onClick={() => setActiveTab('up')}
            className={`px-3.5 py-1.5 rounded-[var(--radius-control)] text-xs font-semibold transition-all ${activeTab === 'up' ? 'bg-white/[0.1] text-content-primary' : 'text-content-subtle hover:text-content-secondary'}`}
          >
            Up() Method (Apply)
          </button>
          <button
            onClick={() => setActiveTab('down')}
            className={`px-3.5 py-1.5 rounded-[var(--radius-control)] text-xs font-semibold transition-all ${activeTab === 'down' ? 'bg-white/[0.1] text-content-primary' : 'text-content-subtle hover:text-content-secondary'}`}
          >
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-content-secondary hover:text-content-primary bg-surface-700 hover:bg-surface-600 px-3 py-1.5 rounded-[var(--radius-control)] transition-all"
          ...
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 text-xs text-content-muted hover:text-content-primary bg-surface-700 hover:bg-surface-600 px-3 py-1.5 rounded-[var(--radius-control)] transition-all"
          >
            {copied ? (
              <>
      ...
      {/* Code Display Area */}
      <div className="relative rounded-[var(--radius-card)] overflow-hidden bg-surface-700">
